...
2. Whenever a new commit is made and the changes are made to airspace.css, run `gulp styles` so that corresponding changes are reflected in minified version of the same file. And when changes are made in main.js or plugins.js files, run `gulp scripts` to make the corresponding changes in the minified versions of the respective files inside **minified** directory. Please note that the commands must be run from the root directory.
3. Few dependencies/libraries do not work well with other dependencies/libraries, so make sure to test before updating the dependencies/libraries. (Here Node js version 8.12.0 LTS is used for minification purpose).
